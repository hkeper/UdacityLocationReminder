...
import com.udacity.project4.util.waitForView
...
class RemindersActivityTest : AutoCloseKoinTest() {// Extended Koin Test - embed autoclose @after method to close Koin after every test
    ...
    @Test
    fun testReminderSavedToastMessage() = runBlocking {
        val activityScenario = ActivityScenario.launch(RemindersActivity::class.java)
        dataBindingIdlingResource.monitorActivity(activityScenario)
        ...
        onView(withId(R.id.gmap)).perform(ViewActions.click())
        ...
        onView(ViewMatchers.withText("Title test")).check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
        onView(ViewMatchers.withText("Desc test")).check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
        activityScenario.close()
    }
